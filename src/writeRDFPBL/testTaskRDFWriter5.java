...
public class testTaskRDFWriter5 {
	
	static String lomvoc = "http://ltsc.ieee.org.rdf/lomvlp0/vocabulary#";
	static String resourceURI = "https://eduknow.org/ORETORNODOROBOFAZEND.md";

	public static void main(String[] args) {
		
		Task task = criaTaskFiniteStateMachineII();
		TaskRDFWriter rdfWriter = new TaskRDFWriter(task, resourceURI);
		
		RDFDataMgr.write(System.out, rdfWriter.getModel(), Lang.RDFXML);
	}
	
	private static Task criaTaskFiniteStateMachineII() {
		Task task = new Task();
		
		task.setTitle("Problema : O Retorno do Robô Fazendeiro.");
		...
		task.addKeyword("Autômato de pilha");
		task.addKeyword("Gramáticas livre de contexto");
		task.addKeyword("Árvores de derivação");
		task.addKeyword("Hierarquia de Chomsky");
		...
		c.setStatement("Desenvolver soluções de problemas usando Autômato de Pilha.");
		...
		c = new Competency();
		c.setStatement("Desenvolver notação baseada em regras.");
		c.setStatementLanguage("pt");
		c.addDisposition(new Disposition("Investigativo"));
		c.addDisposition(new Disposition("Colaborativo"));		
		c.addDisposition(new Disposition("Responsável"));
		...
		c = new Competency();
		c.setStatement("Testar autômatos de pilha usando JFlap.");
		c.setStatementLanguage("pt");
		c.addDisposition(new Disposition("Investigativo"));
		c.addDisposition(new Disposition("Colaborativo"));		
		c.addDisposition(new Disposition("Responsável"));
