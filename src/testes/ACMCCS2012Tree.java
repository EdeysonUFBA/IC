...
public class ACMCCS2012Tree extends JPanel
	...
	private void criaHierarquia(DefaultMutableTreeNode treeNode, ACMCCSConcept skosItem) {
		ACMCCSConcept child;
		...
		if (skosItem.getNarrower().size() > 0) {
		    for (Object code : skosItem.getNarrower()) {
		    	child = rACM.getMapaCCS().get(code);
		    	criaHierarquia(acmTreeNode, child);
		    }			
		}
    ...
    private void createNodes(DefaultMutableTreeNode top) {
	    ...
	    for (String code : rootNodes) {
	    	skosItem = rACM.getMapaCCS().get(code);
	    	criaHierarquia(top, skosItem);
	    }
